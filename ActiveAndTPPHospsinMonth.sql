...
if object_id('tempdb..#Enrs','u') is not null
    drop table #Enrs
select
...
-----------------Monthbase Table   (all months all clinics, no exclusions) -----------------
if object_id('tempdb..#monthbase','u') is not null
    drop table #monthbase
select
...
----------------All the months where a hospital is TPP  (Same as monthbase)-----------------
...
if object_id('tempdb..#FullList', 'u') IS NOT NULL
	drop table #FullList
select TM.datekey
,	TM.clinicid
...
into #FullList
from #TPPinMonth TM
full outer join #Enrs E with (nolock) on (TM.clinicid = E.clinicid AND TM.datekey = E.EnrMo)
group by TM.datekey
,	TM.ClinicId
,	ISNULL(E.Hosps, 0)
,	ISNULL(E.Pets, 0)
...
if object_id('tempdb..#clinicmonth','u') is not null
    drop table #clinicmonth
select
    #FullList.datekey
,   #FullList.clinicid
,	cp.PartnerId
...
into #clinicmonth
from #FullList
	left join dw.ClinicPartner cp with (nolock) on (#FullList.clinicid = cp.ClinicId) AND cp.EndDate IS NULL
    left join #FullList e1 on e1.datekey = #FullList.datekey and e1.clinicID = #FullList.clinicID
    left join #FullList e2 on e2.datekey = dateadd(m,-1,#FullList.datekey) and e2.clinicID = #FullList.clinicID
    left join #FullList e3 on e3.datekey = dateadd(m,-2,#FullList.datekey) and e3.clinicID = #FullList.clinicID
order by #FullList.clinicid, #FullList.datekey
